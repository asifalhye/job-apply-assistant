...
import { getDb, schema, nowIso, normalizeQuestion, getDataDir } from '@jaa/storage';
...
let runnerLock: Promise<void> = Promise.resolve();
...
async function launchBrowser() {
  ...
  const userDataDir = join(getDataDir(), 'browser-profile');
  try {
  ...
  page = activeContext.pages()[0] ?? await activeContext.newPage();
}
...
function withRunnerLock<T>(fn: () => Promise<T>): Promise<T> {
  const task = runnerLock.then(fn, fn);
  runnerLock = task.then(() => undefined, () => undefined);
  return task;
}
...
export async function registerRunnerRoutes(app: FastifyInstance) {
  ...
  app.post('/runner/start', async (req, reply) => {
    ...
    return withRunnerLock(async () => {
      try {
        await launchBrowser();
        await gotoJobUrl(body.jobUrl);

        if (!page) {
          return reply.status(500).send({ error: 'Browser not ready' });
        }
        ...
        return {
          ...
          hint: fields.length === 0
            ? 'Scroll to the Apply form in the browser window, then click Start again.'
            : undefined,
        };
      } catch (err) {
        await closeBrowser();
        ...
        if (isBrowserClosedError(err)) {
          return reply.status(409).send({
            ...
            message: 'Browser closed unexpectedly. Click Start again — a fresh window will open.',
          });
        }
